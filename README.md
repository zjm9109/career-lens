...
- **仅供个人求职决策辅助**；禁止批量商业采数、转售或绕过平台限制  
- 须遵守 Boss / 猎聘 / 智联等用户协议；账号风控与使用后果自行承担  
- **不自动投递、不自动打招呼**；不协助绕过验证码  
- 无自建后端：数据在本地；开启 AI 时摘要发往你自备的模型接口  
- 侧栏首次精筛需勾选同意须知；工具内日风控/开详情限额用于保护账号，请勿通过重装故意规避  
